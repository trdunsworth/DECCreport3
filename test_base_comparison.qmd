...
for (i in 0:(weeks_to_analyze - 1)) {
  ...
  if (file.exists(data_file)) {
    week_df <- vroom::vroom(data_file, show_col_types = FALSE) %>%
      mutate(
...
priority_efficiency_long <- priority_efficiency %>%
  select(Priority_Number, Median_TTQ, Median_TTD, Median_Phone) %>%
...
ggplot(priority_efficiency_long, aes(x = Priority_Number, y = Seconds, fill = Metric)) +
  geom_col(position = "dodge", width = 0.7) +
  geom_text(aes(label = round(Seconds, 0)), 
            position = position_dodge(width = 0.7),
            vjust = -0.5, size = 3) +
  scale_fill_manual(values = c("#3498DB", "#E74C3C", "#16A085")) +
  labs(
    title = "Response Time Metrics by Priority Level",
    subtitle = "Lower priority calls show longer handling times",
    x = "Priority Level",
...
Priority 1 calls have a median Time to Dispatch of `r priority_efficiency %>% filter(Priority_Number == "1") %>% pull(Median_TTD)` seconds, which is `r ifelse(priority_efficiency %>% filter(Priority_Number == "1") %>% pull(Median_TTD) <= 60, "within", "outside")` the SLA target of 60 seconds.

### Call Type Complexity Analysis

```{r call-complexity-scoring}
...
to_ft(
  top_complex %>% 
    select(Problem, Agency, Call_Count, Median_Processing, Mean_Processing, Complexity_Index, Complexity_Category),
...
The most complex call type this week was **`r top_complex$Problem[1]`** with a complexity index of `r top_complex$Complexity_Index[1]`, requiring an average of `r round(top_complex$Mean_Processing[1])` seconds to process.
...
p1_worst_ttq <- p1_calls %>%
  ...
  slice_head(n = 5) %>%
  select(Incident_Number, Agency, Problem, Call_Taker, Time_To_Queue, Time_To_Dispatch)

# Find top 5 worst TTD for P1
p1_worst_ttd <- p1_calls %>%
  arrange(desc(Time_To_Dispatch)) %>%
  slice_head(n = 5) %>%
  select(Incident_Number, Agency, Problem, Dispatcher, Time_To_Queue, Time_To_Dispatch)
