...
## Week 47 from 16 November through 22 November 2025
...
```{r}
#| label: phone-4week-abandoned-plot
#| echo: false
#| message: false
#| warning: false
#| fig-cap: "9-1-1 Abandoned Calls Over Last 4 Weeks"
#| fig-width: 10
#| fig-height: 6

if (!is.null(phone_trends_df) && nrow(phone_trends_df) > 0) {
  
  # Combined bar + line plot for abandoned calls
  p_abandoned <- ggplot(phone_trends_df, aes(x = reorder(week_label, week_start))) +
    geom_col(aes(y = abandoned_911), fill = "#8B0000", width = 0.6, alpha = 0.8) +
    geom_text(aes(y = abandoned_911, label = scales::comma(abandoned_911)), 
              vjust = -0.5, size = 3.5, fontface = "bold") +
    geom_line(aes(y = abandoned_pct * max(abandoned_911) / max(abandoned_pct), group = 1), 
              color = "#8B0000", linewidth = 1.2) +
    geom_point(aes(y = abandoned_pct * max(abandoned_911) / max(abandoned_pct)), 
               color = "#8B0000", size = 3) +
    geom_text(aes(y = abandoned_pct * max(abandoned_911) / max(abandoned_pct), 
                  label = paste0(round(abandoned_pct, 2), "%")), 
              vjust = -1, size = 3, color = "#8B0000") +
    scale_y_continuous(
      name = "Abandoned Call Count",
      sec.axis = sec_axis(~ . * max(phone_trends_df$abandoned_pct) / max(phone_trends_df$abandoned_911), 
                          name = "Abandoned Call Percentage (%)")
    ) +
    labs(title = "9-1-1 Abandoned Calls: Volume and Percentage Over Last 4 Weeks",
         x = "Week") +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
      axis.text.x = element_text(size = 9),
      axis.title.y.left = element_text(color = "#8B0000", face = "bold"),
      axis.title.y.right = element_text(color = "#8B0000", face = "bold"),
      axis.text.y.left = element_text(color = "#8B0000"),
  ...
  cat("*Insufficient phone data for abandoned call trend visualization.*\n\n")
}
```
