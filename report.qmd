...
df_hp <- df[which((df$Agency == "POLICE" & df$Priority_Number < 2) |
                    (df$Agency %in% c("FIRE", "EMS") & df$Priority_Number < 3)), ]
df_law <- df[which(df$Agency == "POLICE")]
df_fire <- df[which(df$Agency == "FIRE")]
df_ems <- df[which(df$Agency == "EMS")]
df_ttq_delay <- df[which(df$Priority_Number < 2 & df$Time_To_Queue > 60)]
df_ttd_delay <- df[which(df$Priority_Number < 2 & df$Time_To_Dispatch > 60)]
df_ca <- df[which(df$Problem == "CARDIAC ARREST ALS 2- SUPV")]