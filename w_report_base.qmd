...
# Source shared plotting helpers (sz function for DOCX-aware sizing)
try({
...
# Set readable Word defaults for all flextable outputs
...
# Helper to produce accessible tables for Word (Office 365)
to_ft <- function(tbl, caption = NULL, header_map = NULL, digits = 2) {
  ...
  # zebra striping
  if (nrow(df) > 0) {
    ft <- flextable::bg(ft, i = seq(1, nrow(df), by = 2), bg = "#F5F5F5", part = "body")
  }
  # header emphasis
...
barDOW <- df |> ggplot(aes(x=DOW, fill=DOW)) +
  ...
  theme(legend.position="none",
        plot.title = element_text(hjust = 0.5, size = sz(18, 14)),
        axis.text.x = element_text(angle=45, hjust=1, size = sz(14, 10)),
        axis.text.y = element_text(size = sz(14, 10)),
        axis.title.x = element_text(size = sz(16, 12)),
        axis.title.y = element_text(size = sz(16, 12)))
...
barHour <- df |> ggplot(aes(x=Hour, fill=Hour)) +
  ...
  theme(legend.position="none",
        plot.title = element_text(hjust = 0.5, size = sz(18, 14)),
        axis.text.x = element_text(angle=45, hjust=1, size = sz(12, 9)),
        axis.text.y = element_text(size = sz(14, 10)),
        axis.title.x = element_text(size = sz(16, 12)),
        axis.title.y = element_text(size = sz(16, 12)))
...
barShift <- df |>
  ...
  theme(legend.position="right",
        ...
        axis.title.x = element_text(size = sz(16, 12)),
        axis.title.y = element_text(size = sz(16, 12)),
        legend.title = element_text(size = sz(14, 11)),
        legend.text = element_text(size = sz(12, 9)))
...
ttq_by_ct <- df_times |>
  dplyr::filter(!is.na(Call_Taker)) |>
  dplyr::group_by(Call_Taker) |>
  dplyr::summarise(
    N_TTQ = sum(!is.na(Time_To_Queue)),
    Total_Time_To_Queue_Sec = sum(Time_To_Queue, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::arrange(dplyr::desc(Total_Time_To_Queue_Sec)) |>
...
phone_by_ct <- df_times |>
  ...
  dplyr::arrange(dplyr::desc(Total_Phone_Time_Sec)) |>
  dplyr::slice_head(n = 10) |>
  dplyr::mutate(
...
ttq_hist_dens <- ggplot(ttq, aes(x = ttq_sec)) +
  ...
  annotate("label", x = ttq_med, y = Inf, vjust = 1.2,
           label = paste0("Median: ", round(ttq_med), "s"), size = sz(4, 3.2), fill = "white") +
  annotate("label", x = ttq_p90, y = Inf, vjust = 1.2,
           label = paste0("P90: ", round(ttq_p90), "s"), size = sz(4, 3.2), fill = "white") +
  annotate("label", x = 15, y = Inf, vjust = 1.2,
           label = "NENA 0:15", size = sz(4, 3.2), fill = "white") +
  annotate("label", x = 20, y = Inf, vjust = 1.2,
           label = "NFPA 0:20", size = sz(4, 3.2), fill = "white") +
  ...
  labs(title = "Time to Queue — Histogram with Density",
    subtitle = paste0("Histogram normalized to density; clipped at 99th percentile (", scales::comma(n_clipped), " removed).\n",
       ...
       caption = "Data: CAD") +
  theme_minimal(base_size = sz(14, 12)) +
  theme(
    plot.title = element_text(face = "bold", size = sz(18, 14)),
    plot.subtitle = element_text(size = sz(14, 12)),
    axis.title.x = element_text(size = sz(16, 12)),
    axis.title.y = element_text(size = sz(16, 12)),
    axis.text.x = element_text(size = sz(12, 10)),
    axis.text.y = element_text(size = sz(12, 10))
  )
...
plot_time_hist_dens <- function(data, var, title_text) {
  ...
  annotate("label", x = med, y = Inf, vjust = 1.2,
    label = "Median", size = sz(3.5, 3), fill = "white") +
  annotate("label", x = p90, y = Inf, vjust = 1.2,
    label = "P90", size = sz(3.5, 3), fill = "white") +
  annotate("label", x = 15, y = Inf, vjust = 1.2,
    label = "NENA", size = sz(3.5, 3), fill = "white") +
  annotate("label", x = 20, y = Inf, vjust = 1.2,
    label = "NFPA", size = sz(3.5, 3), fill = "white") +
    ...
    theme_minimal(base_size = sz(14, 12)) +
    theme(
      plot.title = element_text(face = "bold", size = sz(16, 13)),
      axis.title.x = element_text(size = sz(14, 12)),
      axis.title.y = element_text(size = sz(14, 12)),
      axis.text.x = element_text(size = sz(12, 10)),
      axis.text.y = element_text(size = sz(12, 10))
    )
}
...
tryCatch({
  ...
  median_phone_time_apd <- median_phone_time
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — APD",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — AFD",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — EMS",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
...
to_ft(call_taker_counts, caption = "Frequency of Call Taker in Delayed TTQ Calls (Descending)", header_map = c(Call_Taker = "Call Taker", n = "Count"), digits = 0)
```
...
to_ft(dispatcher_counts, caption = "Frequency of Dispatcher in Delayed TTD Calls (Descending)", header_map = c(Dispatcher = "Dispatcher", n = "Count"), digits = 0)
```
...
tryCatch({
  ...
  filter(Variable == "Time To Dispatch") |>
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — E-911",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
...
tryCatch({
  ...
  filter(Variable == "Time To Dispatch") |>
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — Cardiac Arrest",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
...
tryCatch({
  ...
  filter(Variable == "Time To Dispatch") |>
    ...
    to_ft(
      summary_table,
      caption = "Weekly Elapsed Time Summary Table — Mental Health",
      header_map = c(
        Variable = "Time Metric",
        Minimum = "Min",
        Mean = "Mean",
