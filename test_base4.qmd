...
Operationally, we should look at the busiest hour for the week this week since it is both unexpected, since it's outside the expected busy hours and stands apart from its neighbors.
...
This week, the most common problem nature was `r busiest_ct`. For AFD, the most common was `r agency_top_problem_names["FIRE"]`.
...
However, when we look at the total time on the phone, we see that `r busiest_tc` is not in the top 10 for total time on the phone. So while we are receiving more calls for service from CAD2CAD, the total time spend on those calls is not significantly high. The first table represents the total amount of time that telecommunicators spent on calls. This week, `r most_time_ct` spent the most time on the phone with a total of `r most_time_ct_total` seconds (or `r round(most_time_ct_total / 60, 1)` minutes) and an average of `r most_time_ct_mean` seconds per call. The second table represents the longest averages to get a call into the queue. This week, `r longest_to_queue_ct` had the longest average time to get a call into queue with an average of `r longest_to_queue_ct_mean` seconds per call.
...
tryCatch({
  ...
  apd_p1_calls <- df_apd |>
    dplyr::filter(df_apd$Priority == 1)

  apd_p1_spec <- df_apd |>
    dplyr::filter(df_apd$Priority == 1 && df_apd$Processing_Time <= 120)

  apd_p2_calls <- df_apd |>
    dplyr::filter(df_apd$Priority == 2)

  apd_p2_spec <- df_apd |>
    dplyr::filter(df_apd$Priority == 2 && df_apd$Processing_Time <= 600)

  apd_p3_calls <- df_apd |>
    dplyr::filter(df_apd$Priority == 3)

  apd_p3_spec <- df_apd |>
    dplyr::filter(df_apd$Priority == 3 && df_apd$Processing_Time <= 1200)

  apd_p4_calls <- df_apd |>
    dplyr::filter(df_apd$Priority == 4)

  apd_p4_spec <- df_apd |>
    dplyr::filter(df_apd$Priority == 4 && df_apd$Processing_Time <= 3600)

  apd_p1_compliance_pct <- round((nrow(apd_p1_spec) / nrow(apd_p1_calls)) * 100, 1)
  apd_p2_compliance_pct <- round((nrow(apd_p2_spec) / nrow(apd_p2_calls)) * 100, 1)
  apd_p3_compliance_pct <- round((nrow(apd_p3_spec) / nrow(apd_p3_calls)) * 100, 1)
  apd_p4_compliance_pct <- round((nrow(apd_p4_spec) / nrow(apd_p4_calls)) * 100, 1)
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    fire_hp_calls <- df_fire |>
      dplyr::filter(df_fire$Priority <= 2)

    fire_64_calls <- df_fire |>
      dplyr::filter(df_fire$Priority <= 2 && df_fire$Processing_Time <= 64)

    fire_106_calls <- df_fire |>
      dplyr::filter(df_fire$Priority <= 2 && df_fire$Processing_Time <= 106)

    fire_64_compliance_pct <- round((nrow(fire_64_calls) / nrow(fire_hp_calls)) * 100, 1)
    fire_106_compliance_pct <- round((nrow(fire_106_calls) / nrow(fire_hp_calls)) * 100, 1)
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    ems_hp_calls <- df_ems |>
      dplyr::filter(df_ems$Priority <= 2)

    ems_64_calls <- df_ems |>
      dplyr::filter(df_ems$Priority <= 2 && df_ems$Processing_Time <= 64)

    ems_106_calls <- df_ems |>
      dplyr::filter(df_ems$Priority <= 2 && df_ems$Processing_Time <= 106)
...
As can be seen `r busiest_day_911` was the busiest day for E-911 calls this week.
