...
ttq_lbq <- quantile(df$Time_To_Queue, 0.01, na.rm = TRUE)
ttq_ubq <- quantile(df$Time_To_Queue, 0.99, na.rm = TRUE)
ttd_lbq <- quantile(df$Time_To_Dispatch, 0.01, na.rm = TRUE)
ttd_ubq <- quantile(df$Time_To_Dispatch, 0.99, na.rm = TRUE)
phone_lbq <- quantile(df$Phone_Time, 0.01, na.rm = TRUE)
phone_ubq <- quantile(df$Phone_Time, 0.99, na.rm = TRUE)
proc_lbq <- quantile(df$Processing_Time, 0.01, na.rm = TRUE)
proc_ubq <- quantile(df$Processing_Time, 0.99, na.rm = TRUE)
...
This filter identified `r nrow(outlier_df)` rows that should be investigated as outliers. We can now look at those calls specifically and examine them in detail to see why these have been collected in our outlier set.
