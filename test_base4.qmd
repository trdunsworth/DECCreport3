...
# Week 41 should be Oct 05-11, 2025 (Sunday to Saturday)
# Sunday-first week calculation

# Method 1: Direct date specification for week 41
if(WEEK_NUMBER == 43) {
  WEEK_START_DATE <- as.Date("2025-10-19")  # Sunday Oct 05
  WEEK_END_DATE <- as.Date("2025-10-25")    # Saturday Oct 11
} else {
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    fire_64_compliance_pct <- if (nrow(fire_hp_calls) > 0) round((nrow(fire_64_calls) / nrow(fire_hp_calls)) * 100, 2) else 0
    fire_106_compliance_pct <- if (nrow(fire_hp_calls) > 0) round((nrow(fire_106_calls) / nrow(fire_hp_calls)) * 100, 2) else 0
...
tryCatch({
  ...
  if (length(existing_columns) == 0) {
    ...
    ems_64_compliance_pct <- if (nrow(ems_hp_calls) > 0) round((nrow(ems_64_calls) / nrow(ems_hp_calls)) * 100, 2) else 0
    ems_106_compliance_pct <- if (nrow(ems_hp_calls) > 0) round((nrow(ems_106_calls) / nrow(ems_hp_calls)) * 100, 2) else 0
...
This section will analyze calls that arrived to DECC via E-911 trunk lines. Last wee, the median processing time for high-priority calls was `r median_processing_time_hp` seconds. 
...
This filter identified `r nrow(outlier_df)` rows that should be investigated as outliers. We can now look at those calls specifically and examine them in detail to see why these have been collected in our outlier set. If we wish to separate out each of the different metrics, we can separate out each of the four elapsed time measures and look at their specific details. For example, there are `r nrow(ttq_outliers)` rows where the Time To Queue value was considered an extereme value by all four tests.
...
As you can see, the outliers have a considerable impact on the median values that we use. That impact is even greater when using the means since there are larger variances in the means and standard deviations compared to the medians and the median absolute deviations.

## Conclusion

This report has covered various aspects of the calls for service during week `r WEEK_NUMBER`. We have analyzed the data for completeness and accuracy, explored it for insights into call patterns and trends, and focused on specific areas of interest such as high-priority calls and cardiac arrest incidents. The findings will assist in making informed decisions to improve service delivery and operational efficiency. This report is under construction to be converted into a web access format.
